...
# Min over block
function gsq_rule_identityH(blocks, alpha, X, y, C, kernel, w_old, b_old)
    updates = zeros(size(blocks)[1])
    # use the largest eigen value for picking
    H = (y * y').*(X * X')
    val = eigvals(H)
    approx = val[1]*Matrix{Float64}(I, size(H)[1], size(H)[1])
    for i = 1:size(blocks)[1]
        updates[i] = gsq_block(blocks[i,:], alpha, X, y, C, approx, kernel, w_old, b_old)
    end
    min = minimum(updates)
...
# Fit function
function fit_gsq_identityHess(X, y, X_test, y_test, kernel, C, epsilon, max_iter)
    ...
    while true
        ...
        count_ += 1
        # compute best block
        best_block, idx = gsq_rule_identityH(blocks, alpha, X, y, C, kernel, w, b)
