...
# dykstra
function dykstra_proj(alpha_b, A, max_iter)
	# initialize counter
	counter = 0
	# run iterative projection
	while true
		# project onto box
		alpha_lower = findall(0 .>  alpha_b)
		alpha_higher = findall(C .<  alpha_b)
		alpha_other = findall((alpha_b .> 0) .& (alpha_b .< C))
		alpha_b[alpha_lower] = zeros(size(alpha_lower))
		alpha_b[alpha_higher] = ones(size(alpha_higher))
		alpha_b[alpha_other] = alpha_b[alpha_other]
		# project onto null
		alpha_b = (Matrix{Float64}(I, length(alpha_b), length(alpha_b)) - A * inv(A'*A) * A') * alpha_b
		# convergence criteria
		if length(findall((alpha_b .> 0) .& (alpha_b .< C))) == alpha_b
