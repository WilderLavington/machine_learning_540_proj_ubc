...
function train_test_split(path, train_test_split = 0.7)
    # load the data
    data = CSV.read(path)
    # split to features and response
    X = convert(Array, data[2:end])
    y = convert(Array, data[1])
    # get the size of the data
    n = Int(length(y))
    # split to train test
    rows = shuffle(collect(1:n))
    # create split
    train_data_size=Int(floor(train_test_split*n))
    # set up the split - X
    X_train = X[rows[1:train_data_size], :]
    X_test = X[rows[train_data_size+1:end], :]
    # set up the split - y
    y_train = y[rows[1:train_data_size], :]
    y_test = y[rows[train_data_size+1:end], :]
    #
...
function data_set_4()
    # set path
    path = "./dataset/australian.csv"
    # create a train test split
    X_train, y_train, X_test, y_test = train_test_split(path, train_test_split = 0.7)
    # return it
    return X_train, y_train, X_test, y_test
end
